...
# Short-Description: jobDaemon
# Description:       Start the RECOVER job daemon. 
### END INIT INFO
# 
...
test -d $JD_PATH || { echo "$JD_PATH does not exist"; 
	if [ "$1" = "stop" ]; then exit 0;
	else exit 5; fi; }

test -e $MGMT_SCRIPT || { echo "$MGMT_SCRIPT does not exist"; 
	if [ "$1" = "stop" ]; then exit 0;
	else exit 5; fi; }

test -e $JD_SCRIPT_TEST_PATH || { echo "$JD_SCRIPT_TEST_PATH does not exist"; 
	if [ "$1" = "stop" ]; then exit 0;
	else exit 5; fi; }
...
case "$1" in
	...
	;;
    stop)
	echo -n "Shutting down the Job Daemon "
	...
	echo -n "Reload service Wrangle (not implemented, does nothing) "
	## if it supports it:
	#killproc -HUP $IRODS_BIN
	...
	;;
    status)
	echo -n "Checking for service Wrangle "
